...
function Scan-Directory
{
    ...
    if ($recurseYes) 
    {
        ...
        $AllFiles = Get-ChildItem $dirName -File -Force -recurse | Where-Object {$_.FullName -notlike "*HashFile*.csv" -or ($isLocked = Test-FileIsLocked $_.FullName) -eq $false} |
...
function compare-data2
{
	...
	foreach ($file in $currentLoad) 
    {
		if ((-not $previousDict.ContainsKey($file.HashValue)) -and (-not $previousDict.ContainsKey($file.FullNameHash)))
        {
            ...
            #Write-Output $file
            #Read-Host "Pausing in file global:filelist"
		}
	}
    ...
    if ($ViewGUI -eq $false)
    {
		[string]$writechange = Read-Host "Do you want to view the view in the GUI(Y/n)?"
		Write-Host "writechange = $writechange"
	
		if ($writechange.ToUpper() -eq  "Y")
		{
			Display-GridViews
		}
    }
    ...
    # function to write change file to disk
    # allows user to decide if they want to 
    # or not
...
function help-menu
{
    ...
    Write-Host "the oldest file is the base file.  It is a switch parameter.  It assumes "
...
# global variable to hold all the data.  It is an array
# of objects
$global:filelist = @()
...
if ($returnedvalue.FileCreated)
{
    ...
    # the scan.  
    Export-FileOut $dirName2 $returnedvalue.ArrayOrRecords
}
else
{
...
{
    write-host "hash file does not exist"
    exit
}

# gets a list of all the HashFile*.csv files
# so the user can compare against the file 
# they just created.  If the user does not choose from the
# list, the value returned is 0
if ((-not $Compare) -or ($Compare))
{
    $fileToLoadArray = get-HashFileList $dirname $base

    if ($($fileToLoadArray[0]) -eq "nofile")
    {
        write-host "done, the user did not want to compare a file"
    }
    elseif ($($fileToLoadArray[0]) -eq "base")
    {
        Write-Host "creating base file"
        $baseValue = $fileToLoadArray[0]
    ...
    else
    {
        ...
        #$global:filelist | Out-GridView
        compare-data2 $previousLoad $returnedvalue.ArrayOrRecords $dirname $dirName2 $ViewGUI
    }
}
